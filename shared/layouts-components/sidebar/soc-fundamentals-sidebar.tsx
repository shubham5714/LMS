...
type SocFundamentalsSidebarProps = {
  onBackToMainNav: () => void
}

/** Stroke-only circle + check (matches outlined “completed” style). */
...
export default function SocFundamentalsSidebar({ onBackToMainNav }: SocFundamentalsSidebarProps) {
  ...
  const loadProgress = useCallback(async () => {
    ...
    const ids = await fetchCompletedTopicIds(user.id, SOC_FUNDAMENTALS_COURSE_ID)
  ...
  useEffect(() => {
    ...
    window.addEventListener(SOC_COURSE_PROGRESS_UPDATED_EVENT, onUpdated)
    return () => window.removeEventListener(SOC_COURSE_PROGRESS_UPDATED_EVENT, onUpdated)
  }, [loadProgress])
    ...
    <Fragment>
      ...
      <aside className="app-sidebar app-sidebar--course sticky d-flex flex-column" id="sidebar">
        <div className="main-sidebar-header">
          <Link scroll={false} href="/courses/soc-fundamentals" className="header-logo">
          ...
          <nav className="main-menu-container nav nav-pills flex-column sub-open flex-fill">
            <ul className="main-menu">
              ...
              <li className="slide__category">
                <span className="category-name">SOC Fundamentals</span>
              </li>
              {SOC_FUNDAMENTALS_TOPICS.map((item) => {
                const active =
                  item.id === "overview"
                    ? pathname === item.path
